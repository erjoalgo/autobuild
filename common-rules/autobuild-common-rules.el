  ...
  (when (file-exists-p "a.out")
    ...
    (lambda ()
      (let* ((gcc-flags "-g -static")
             (build-cmd (autobuild-c++)))
        (compile build-cmd))
      (compile (format "valgrind --track-origins=yes --leak-check=full ./a.out")))))

(autobuild-define-rule autobuild-c++-perf (c++-mode)
  (autobuild-nice 13)
  (lambda ()
    (let* ((gcc-flags "-O3 -fno-omit-frame-pointer -g")
           (build-cmd (autobuild-c++)))
      (compile build-cmd))
    (compile "perf record --call-graph dwarf ./a.out")
    (compile "perf report" t)))
