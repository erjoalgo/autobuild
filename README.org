* Overview
  ...
  emacs-lisp functions, to asynchrousl build pipelines that chain simpler
...
** ~autobuild-define-rule~
   ...
   the user to select. Otherwise, it must reutrn ~nil~.
...
** ~autobuild-pipeline~
   ...
   Within a piepile, if a rule's action is either a string (denoting a compile-command)
